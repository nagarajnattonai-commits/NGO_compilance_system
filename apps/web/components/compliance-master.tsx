...
export default function ComplianceMaster() {
  ...
  return <ComplianceMasterShell><div className="master-heading"><div><h1>{t("title")}</h1><p className="account-intro">{t("intro")}</p></div>
    <Link className="button primary" href="/admin/compliance-master/new"><Plus size={18} />{t("create")}</Link></div>
    ...
    <section className="card master-filters"><label className="master-search"><Search size={17} /><span className="sr-only">{t("search")}</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("search")} /></label>
      <div className="master-form-grid">
        <label>{t("status")}<select value={filters.status || ""} onChange={(e) => filter("status", e.target.value)}><option value="">{t("all")}</option>{templateStatuses.map((value) => <option key={value} value={value}>{t(`states.${value}`)}</option>)}</select></label>
        <label>{t("category")}<select value={filters.category_id || ""} onChange={(e) => filter("category_id", e.target.value)}><option value="">{t("all")}</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
        <label>{t("frequency")}<select value={filters.frequency || ""} onChange={(e) => filter("frequency", e.target.value)}><option value="">{t("all")}</option>{frequencies.map((value) => <option key={value} value={value}>{t(`frequencies.${value}`)}</option>)}</select></label>
        <label>{t("risk")}<select value={filters.risk_level || ""} onChange={(e) => filter("risk_level", e.target.value)}><option value="">{t("all")}</option>{risks.map((value) => <option key={value} value={value}>{t(`risks.${value}`)}</option>)}</select></label>
        ...
        <label>{t("sort")}<select value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}>{["name", "code", "category", "updated_at", "version", "status"].map((key) => <option key={key} value={key}>{t(key)}</option>)}</select></label>
        <label>{t("order")}<select value={order} onChange={(e) => setOrder(e.target.value)}><option value="asc">{t("ascending")}</option><option value="desc">{t("descending")}</option></select></label>
      ...
      {data?.items.map((row) => <tr key={row.id}>
        <td><Link className="master-truncate" title={row.name} href={`/admin/compliance-master/${row.id}`}>{row.name || t("untitled")}</Link></td><td><code>{row.code}</code></td><td><span className="master-truncate" title={row.category}>{row.category}</span></td><td>{row.jurisdiction}</td><td>{row.organization_types === "ALL" ? t("all") : row.organization_types === "RULES" ? t("configuredRules") : row.organization_types}</td><td>{t(`frequencies.${row.frequency}`)}</td><td>v{row.version}{row.current_version && row.current_version !== row.version && <small>{t("publishedVersion", { version: row.current_version })}</small>}</td><td>{t(`states.${row.status}`)}</td><td>{formatDateTime(row.updated_at, { locale })}</td><td>{row.updated_by}</td><td>
          <details className="master-row-actions"><summary>{t("actions")}</summary><div>
            <Link href={`/admin/compliance-master/${row.id}`}>{t(row.status === "DRAFT" ? "edit" : "view")}</Link>
            ...
            <button disabled={busy} onClick={() => void action(row, "clone")}>{t("clone")}</button>
            {row.status === "DRAFT" && <button disabled={busy} onClick={() => void action(row, "submit-review")}>{t("submit")}</button>}
            {row.status === "UNDER_REVIEW" && <button disabled={busy} onClick={() => void action(row, "approve")}>{t("approve")}</button>}
            {row.status === "APPROVED" && <button disabled={busy} onClick={() => void action(row, "publish")}>{t("publish")}</button>}
            {["PUBLISHED", "ARCHIVED"].includes(row.status) && <button disabled={busy} onClick={() => void action(row, "new-version")}>{t("newVersion")}</button>}
            {row.status !== "ARCHIVED" && <button disabled={busy} onClick={() => void action(row, "archive")}>{t("archive")}</button>}
          </div></details>
        </td></tr>)}
    </tbody></table></div>
