...
import { actOnTemplate, emptyTranslation, getTemplate, newConfiguration, templatePath, type ApplicabilityPreview, type ComplianceCategory, type ComplianceTemplate, type MasterMetadata, type TemplateConfiguration, type TemplateTranslation } from "@/lib/compliance-master";
...
function Field({ label, value, onChange, type = "text", multiline = false, min, max, maxLength = 4000 }: { label: string; value: string | number; onChange: (value: string) => void; type?: string; multiline?: boolean; min?: number; max?: number; maxLength?: number }) {
  return <label>{label}{multiline ? <textarea rows={4} maxLength={maxLength} value={value} onChange={(e) => onChange(e.target.value)} /> : <input type={type} value={value} min={min} max={max} maxLength={maxLength} onChange={(e) => onChange(e.target.value)} />}</label>;
}
...
export default function ComplianceTemplateBuilder({ id }: { id?: string }) {
  ...
  const readOnly = row !== null && row.status !== "DRAFT";
  ...
  function update(patch: Partial<TemplateConfiguration>) { setConfiguration((old) => ({ ...old, ...patch })); setDirty(true); setValidation(null); setPreview(null); setNotice(""); }
  ...
  function roleSelect(label: string, value: string, onChange: (value: string) => void) { return <label>{label}<select value={value} onChange={(e) => onChange(e.target.value)}>{metadata?.roles.map((role) => <option key={role} value={role}>{t(`roles.${role}`)}</option>)}</select></label>; }
  async function save() {
  ...
  const translated: TemplateTranslation = configuration.translations[editingLocale] || emptyTranslation();
  ...
  return <ComplianceMasterShell dirty={dirty}><div className="master-heading"><div><h1>{row ? row.configuration.name || t("untitled") : t("create")}</h1><p>{code}{row && <> · v{row.version} · {t(`states.${row.status}`)}</>}</p></div><div className="master-actions">
    ...
    {row && ["PUBLISHED", "ARCHIVED"].includes(row.status) && <button className="button secondary" disabled={busy} onClick={() => void action("new-version")}>{t("newVersion")}</button>}
  </div></div>
    ...
    {loading || navigating ? <p role="status">{t("loading")}</p> : <div className="master-builder-layout">
      <nav className="master-step-nav" aria-label={t("steps")}>
        <label className="master-mobile-step">{t("steps")}<select value={step} onChange={(e) => setStep(Number(e.target.value))}>{steps.map((key, index) => <option key={key} value={index}>{index + 1}. {t(`stepsList.${key}`)}</option>)}</select></label>
        ...
        <fieldset disabled={readOnly || busy}><legend className="sr-only">{t(`stepsList.${steps[step]}`)}</legend>
          {step === 0 && <div className="master-form-grid">
            ...
            <label>{t("code")}<input value={code} disabled={!!id} maxLength={40} onChange={(e) => { setCode(e.target.value.toUpperCase()); setDirty(true); }} /><small>{t("codeHelp")}</small></label>
            <label>{t("category")}<select value={configuration.category_id} onChange={(e) => update({ category_id: e.target.value })}><option value="">{t("select")}</option>{categories.filter((c) => c.enabled || c.id === configuration.category_id).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          ...
          {step === 1 && <>
            ...
            {!a.match_all && <><label>{t("combineGroups")}<select value={a.operator} onChange={(e) => update({ applicability: { ...a, operator: e.target.value as "AND" | "OR" } })}><option value="AND">{t("and")}</option><option value="OR">{t("or")}</option></select></label>
              <ComplianceMasterSortable disabled={readOnly} items={a.groups} onChange={(groups) => update({ applicability: { ...a, groups } })} render={(group, groupIndex) => <>
                ...
                <ComplianceMasterSortable disabled={readOnly} items={group.conditions} onChange={(conditions) => update({ applicability: { ...a, groups: replace(a.groups, groupIndex, { conditions }) } })} render={(condition, conditionIndex) => {
                  ...
                  return <div className="master-form-grid"><label>{t("field")}<select value={condition.field} onChange={(e) => change({ field: e.target.value, operator: "EQUALS", value: metadata?.fields[e.target.value] === "boolean" ? true : "" })}>{Object.keys(metadata?.fields || {}).map((field) => <option value={field} key={field}>{t(`fields.${field}`)}</option>)}</select></label>
                    <label>{t("operator")}<select value={condition.operator} onChange={(e) => change({ operator: e.target.value, value: ["IN", "NOT_IN"].includes(e.target.value) ? [] : datatype === "boolean" ? true : "" })}>{metadata?.operators[datatype]?.map((op) => <option value={op} key={op}>{t(`operators.${op}`)}</option>)}</select></label>
                    {!condition.operator.startsWith("IS_") && (datatype === "boolean" ? <label>{t("value")}<select value={String(condition.value)} onChange={(e) => change({ value: e.target.value === "true" })}><option value="true">{t("yes")}</option><option value="false">{t("no")}</option></select></label> : <Field label={t("value")} type={datatype === "number" ? "number" : datatype === "date" ? "date" : "text"} maxLength={200} value={Array.isArray(condition.value) ? condition.value.join(", ") : String(condition.value ?? "")} onChange={(value) => change({ value: ["IN", "NOT_IN"].includes(condition.operator) ? value.split(",").map((s) => s.trim()).filter(Boolean) : datatype === "number" ? value === "" ? null : Number(value) : value })} />)}
          ...
          {step === 2 && <div className="master-form-grid">
            <label>{t("frequency")}<select value={configuration.recurrence.frequency} onChange={(e) => update({ recurrence: { ...configuration.recurrence, frequency: e.target.value } })}>{frequencies.map((value) => <option key={value} value={value}>{t(`frequencies.${value}`)}</option>)}</select></label>
            ...
            <label>{t("deadlineStrategy")}<select value={configuration.deadline.strategy} onChange={(e) => update({ deadline: { ...configuration.deadline, strategy: e.target.value } })}>{deadlineStrategies.map((value) => <option key={value} value={value}>{t(`deadlines.${value}`)}</option>)}</select></label>
          ...
          {step === 7 && <><p>{t("reminderHelp")}</p><ComplianceMasterSortable disabled={readOnly} items={configuration.reminders} onChange={(reminders) => update({ reminders })} render={(item, index) => <div className="master-form-grid">
            ...
            <Field label={t("escalationLevel")} type="number" min={0} max={10} value={item.escalation_level} onChange={(value) => update({ reminders: replace(configuration.reminders, index, { escalation_level: Number(value) }) })} /><label>{t("channel")}<select value={item.channel} onChange={() => undefined}><option value="IN_APP">{t("inApp")}</option></select></label>
          ...
          {step === 9 && <><label>{t("locale")}<select value={editingLocale} onChange={(e) => setEditingLocale(e.target.value)}>{supportedLocales.map((language) => <option key={language.code} value={language.code}>{language.nativeLabel}</option>)}</select></label><p>{t("translationHelp")}</p><div className="master-form-grid">
        ...
        {[1, 2, 10].includes(step) && <section className="master-preview"><h3>{t("testApplicability")}</h3><div className="master-form-grid"><Field label={t("organizationSearch")} value={organizationSearch} onChange={setOrganizationSearch} /><button type="button" className="button secondary" disabled={busy} onClick={() => void findOrganizations()}>{t("search")}</button><label>{t("sampleOrganization")}<select value={organizationId} onChange={(e) => setOrganizationId(e.target.value)}><option value="">{t("select")}</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name} · {organization.tenant_id}</option>)}</select></label><Field label={t("asOf")} type="date" value={asOf} onChange={setAsOf} /><Field label={t("eventDate")} type="date" value={eventDate} onChange={setEventDate} /></div><button type="button" className="button secondary" disabled={busy || !organizationId} onClick={() => void testApplicability()}>{t("testApplicability")}</button>
        ...
        {step === 10 && <>
          <div className="master-actions"><button className="button secondary" disabled={busy || !row || dirty} onClick={() => void action("validate")}>{t("validate")}</button>
            {row?.status === "DRAFT" && <button className="button primary" disabled={busy || dirty} onClick={() => void action("submit-review")}>{t("submit")}</button>}
            {row?.status === "UNDER_REVIEW" && <button className="button primary" disabled={busy} onClick={() => void action("approve")}>{t("approve")}</button>}
            {row && ["UNDER_REVIEW", "APPROVED"].includes(row.status) && <button className="button secondary" disabled={busy} onClick={() => void action("request-changes")}>{t("requestChanges")}</button>}
            {row?.status === "APPROVED" && <button className="button primary" disabled={busy} onClick={() => void action("publish")}>{t("publish")}</button>}
            {row && row.status !== "ARCHIVED" && <button className="button secondary" disabled={busy || dirty} onClick={() => void action("archive")}>{t("archive")}</button>}
          </div>
