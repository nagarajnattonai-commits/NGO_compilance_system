      ...
      <nav className="master-step-nav" aria-label={t("steps")}>
        ...
        <fieldset disabled={readOnly || busy}><legend className="sr-only">{t(`stepsList.${steps[step]}`)}</legend>
          ...
          {step === 7 && <><p>{t("reminderHelp")}</p><ComplianceMasterSortable disabled={readOnly} items={configuration.reminders} onChange={(reminders) => update({ reminders })} render={(item, index) => <div className="master-form-grid">
            ...
            <Field label={t("escalationLevel")} type="number" min={0} max={10} value={item.escalation_level} onChange={(value) => update({ reminders: replace(configuration.reminders, index, { escalation_level: Number(value) }) })} /><label>{t("channel")}<select aria-label={t("channel")} value={item.channel} onChange={(event) => update({ reminders: replace(configuration.reminders, index, { channel: event.target.value as "IN_APP" | "EMAIL" | "WHATSAPP" }) })}>{metadata?.channels.map((channel) => <option key={channel} value={channel}>{channel === "IN_APP" ? t("inApp") : channel === "EMAIL" ? "Email" : "WhatsApp"}</option>)}</select></label>
            {item.channel === "WHATSAPP" && <Field label="Approved WhatsApp template (optional)" maxLength={100} value={item.provider_template || ""} onChange={(provider_template) => update({ reminders: replace(configuration.reminders, index, { provider_template }) })} />}
          ...
          </div>} /><button className="button secondary" type="button" onClick={() => update({ reminders: [...configuration.reminders, { id: itemId(), offset_days: 0, channel: "IN_APP", recipient_role: configuration.responsibility.owner_role, escalation_level: 0, enabled: true, text: "", provider_template: "" }] })}>{t("addReminder")}</button></>}
