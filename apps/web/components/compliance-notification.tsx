...
export function ComplianceNotificationTitle({ item }: { item: Notification }) {
  const locale = useLocale(); const variables = item.template_variables;
...
export function ComplianceNotificationMessage({ item }: { item: Notification }) {
  const locale = useLocale(); const t = useTranslations("ComplianceMaster"); const variables = item.template_variables;
